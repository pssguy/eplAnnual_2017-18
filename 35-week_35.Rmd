...
embed_youtube("X0xoyvCJDyw", width = 420, height = 315, frameborder = 0,
...
```{r}  
shortcode("tweet", "986892971827253250") 
```
...
data %>%
  ...
  layout(
    ...
    yaxis = list(title = ""),
    xaxis = list(title = "Maximum Gap between Goals (mins)")
  ) %>% config(displayModeBar = F, showLink = F)
...
![copy of final weeks PSS stuff]()


#### Arsene Wenger {-}

Another disappointment as defeat in the Europa Cup Final means that Arsenal miss out on Champions League football again in 2018/19

Here is Wenger's record against all other teams
...
No real surorise, though in admittedly small sample siazes he did not perform that well 
 against the Sheffield Clubs
 
---
 
 And by opposingmanager
