...
df <- standings %>%
  filter(season < "1995/96" &
           tmYrGameOrder == 42 | season > "1994/95" & tmYrGameOrder == 34) %>%
...
```
 ...
 West Ham and Sunderland are the two teams that would have been relegated if the decision was based on goal difference but actually finished 15th
...
```{r}  
shortcode("tweet", "975682892335591426") 
```
...
<p class="factoid">Mane grabs three assists for first time in EPL career and is just 2 goals shy of a double-double for third consecutive season</p>
...
Milivojevic did finish as top penalty scorer with 7, allowing Benteke to take one when they were 4-0 up on Leicester in injury time, three clear of three others (including Murray who missed a couple for Brighton)


---

Mane duly got his double-double, one of just ..  players to achieve the feat
