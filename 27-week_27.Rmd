...
Aguero finished the season just above the 50% mark and reached the 20 goal mark for the fourth year in succession (..) as he finally made the PFA team of the season

Kane's multi-goal scoring explots dropped a bit as his final .. were single goal exploits 
...
There was at least one draw in every round ..
...
standings %>%
  filter(season == "2017/18", res == "Draw") %>%
  count(tmYrGameOrder, sort = TRUE) %>%
  mutate(draws = as.factor(n / 2)) %>%
  group_by(draws) %>%
  tally()
